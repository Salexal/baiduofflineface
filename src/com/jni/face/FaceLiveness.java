...
public class FaceLiveness {
    ...
    public int faceLivenessByRgbImage() {
        ...
        TrackFaceInfo trackInfo = liveInfos[0].trackinfo;
        if (trackInfo == null) {
        ...
        // 人脸框宽度
        System.out.println("face width is:" + trackInfo.box.width);
        // 人脸框高度
        System.out.println("face height is:" + trackInfo.box.height);
        // 人脸角度
        System.out.println("face angle is:" + trackInfo.box.angle);
        // 人脸框中心x坐标
        System.out.println("face center x is:" + trackInfo.box.centerx);
        // 人脸框中心y坐标
        System.out.println("face center y is:" + trackInfo.box.centery);
        // 人脸置信度
        System.out.println("face score is:" + trackInfo.box.score);
            ...
            if (!frame.empty()) {
                ...
                if (liveInfos != null && liveInfos.length > 0) { 
                    for (int j = 0; j < liveInfos.length; j++) {
                        TrackFaceInfo info = liveInfos[j].trackinfo;
            ...
            if (!frame1.empty() && !frame2.empty()) {
                ...
                if (rgbLives != null && irLives != null && rgbLives.length > 0 
                    ...
                    for (int j = 0; j < rgbLives.length; j++) {
                        TrackFaceInfo info  = rgbLives[j].trackinfo;
    ...
    public void rgbDepthLivenessByOrbe() {
        ...
        System.loadLibrary(Core.NATIVE_LIBRARY_NAME); // 打开摄像头或者视频文件 VideoCapture capture1 = new VideoCapture();
        ...
        int radius = 2;
        while (!stop) {
            int bopen = Face.cameraOrbe(rgbAddr, depthAddr);
            if (bopen != 0) {
            ...
            if (!cvRgb.empty() && !cvDepth.empty()) {
                ...
                if (infos != null && infos.length > 0) {
                    for (int j = 0; j < infos.length; j++) {
                        TrackFaceInfo info  = infos[j].trackinfo;
    ...
    public void rgbDepthLivenessByHjimi() {
        ...
        boolean stop = false;
        while (!stop) {
            int bopen = Face.cameraHjimi(rgbAddr, depthAddr);
            if (bopen != 0) {
                System.out.println("read frame fail!");
                continue;
            } 
